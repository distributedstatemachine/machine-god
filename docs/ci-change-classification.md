...
When an Apple matrix job selects native or CLI tests (or the whole workspace), it
...
fixture capability. CLI-only selections require this prerequisite too, without
selecting native package tests. The serial tests therefore exercise shipped
private helper dispatch instead of using the large libtest executable as the default helper.
