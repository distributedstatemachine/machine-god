...
- Active phase: `bounded top-level ask CLI cycle-2 candidate is rejected and remediation is active`
- Next gate: `integrate every cycle-2 finding, pass the complete exact-1.94.1 replacement gate, then restart all three review tracks on one fresh candidate`
