...
replacement to permit stale B delivery. Cycle-8 docs `22d5702`/`3650dba`,
evidence `cf4abfd`/`5681bab`, and source `a1b3d23`/`d8075ff` compose at exact
head `d8075ff`/`fa32564`. The lane/activity remains retained while A is destroyed
under `catch_unwind` outside the lock; afterward, replay arbitration observes
the current lifecycle/pending/target state. Destructor close/replacement wins,
callback or target-drop panic clears the lane, callback panic wins when both
occur, foreign work stays outside the lock, and callback concurrency remains at
most one. Both deterministic regressions and the complete local gate are green.
...
lifecycle/platform and performance/resources were `0/0/1/0` each, and the
union is `0/0/2/0`. A secondary panic payload can panic during
destruction and replace the selected callback panic; independently, synchronous
outer re-poll plus re-notify can extend one activation to 257 callbacks for
...
docs `d839d18`/`f8342db`, evidence `3692d19`/`adf2b93`, and source `83dd836`
compose at exact head `b8b721a065f4b14f5f3678a22ee5b0bd2267ca2f`, tree
`46721503429685e5feb8e4ac33f74e865acf0c2a`. A state-owned lifetime budget
accounts every callback before dispatch and reaches sticky terminal 256 across
...
forever. Cycle-12 docs `5047d40`/`e582331`, source `0684f3e`/`54d0af0`, and
...
`8378a47`/`522d0a4`. Release panic is `unwind`. Capacity lives in detachable
state; admitted callbacks and close hold local guards through callback/target
teardown, after which closed retained Waker identities own no permit. The
release probe records prompt-poll recovery, while ordinary/ambient retained-
Waker evidence records the cleanup paths under the test profile.
Direct 46, manifest eight, all exact/extended/portability gates, and the fresh
release/smokes are green. Formal cycle 12 rejected exact candidate
`3dec7a2f073fa85479af19765b03b06cdfd9da8c`, tree
`c34d20a45f70b82652bf78df9653f39399d7fc6d`, with track counts `0/0/1/1`,
...
prior findings are resolved. Review-exempt delivered record `490d122`, tree
`fe8c07f`, passed feature CI/Benchmark runs `33075442826`/`33075442823` and
main CI/Benchmark runs `33076056430`/`33076056448`; every workflow succeeded
for that exact SHA and each benchmark run retained exactly two unexpired exact-
SHA artifacts. Slice 35 is delivered and the delivered count is 35. This
documentation-only delivery seal is review-exempt and changes no product
...
`0/0/0/1`, deduplicated to one low stale cross-document-summary finding. That
...
reviews each reported 0 blocker, 0 high, 0 medium, and 0 low findings, so their
...
catalog types remain available without either HTTP feature.
...
2 medium, and 1 low. It covers blocking per-query entropy within the total
deadline, missing cancellation/deadline authority at native pre-effect
boundaries, and one duplicated cancellation-waiter finding repeated across two
tracks. Candidate `16f5afe` is rejected. Exact isolated production remediation
component `9abef298352ea3d9517543c384d9703b949cda75`, tree
`b1ad6f79a9de3414d87c9ff01b28e8c216e6b676`, changes only
`crates/machine-god-native/src/web_fetch.rs`. It supplies the construction-time
32-byte key, `AtomicU32` counter and bounded SHA-256 query-ID derivation,
carried before/after native-effect deadline checks, and one cancellation owner.
...
cancellation wake, a cancelled result, and pending owned-work drop/release in
...
deduplicates across correctness and performance, leaving 0 blocker, 0 high,
1 medium, and 1 low; the exact candidate is rejected. The medium finding is a
same-poll DNS TCP-connect defect: a ready connect result could escape when the
configured connect deadline became due during that effect poll. The replacement
must preserve cancellation and outer-deadline precedence, then reject an
...
permit through rendering and its final boundary. Its single cancellation
waiter and single outer machine-god invocation-deadline sleep cover bounded
permit/DNS/HTTP/body waits; the final synchronous boundary checks the
token/deadline directly and creates no second waiter. Each truncated A or AAAA
DNS TCP replay may additionally own one short-lived configured connect-timeout
sleep, so an invocation owns at most two sequential DNS replay sleeps.
...
begins. None resets or extends the outer absolute deadline. Native construction
synchronously snapshots the first
system-configured UDP resolver and one random query-ID seed outside invocation
timing. Hostname execution uses the stored nameserver and derives each A/AAAA
...
deadline at every
pre-effect transition between A, AAAA, TCP replay, HTTP dispatch, and body
...
evidence compose at `d8075ff`/`fa32564`: A is destroyed outside the lock before
then-current replay arbitration, and panic clears the lane. The focused and
complete local gates were green. Formal cycle 8 rejected exact
...
cycle 13 is green on `a4f1bb9`/`72a0303` with a zero-finding union. Exact
delivery record `490d122` passed feature and main CI/Benchmark gates; slice 35
is delivered and the delivered count is 35.
...
workspace-descriptor clone. Cycle 2 intentionally inserts `answer` before
`question` independent of map implementation or feature unification, adds the
...
cycle 13 is green on `a4f1bb9`/`72a0303` with a zero-finding union. Exact
delivery record `490d122` passed feature and main CI/Benchmark gates; slice 35
is delivered and the delivered count is 35. The analogous preexisting
