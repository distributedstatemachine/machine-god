...
The validated configured model is retained and used by both the ordinary AI
Gateway provider and the web-search transport adapter. The host retains the
complete `LoadedNativeConfig`, including its observable schema origin/version.
...
actually contacts; that value becomes web-search permission identity. The
custom path is a trusted authority override and reports no discovered
credential source.
...
- construct provider and web-search adapters over the shared transport; and
- construct `web_fetch`, including its bounded native resolver/entropy setup.

Later provider, web-fetch, and web-search polling requires a compatible
