...
use machine_god_core::{
    ...
    ToolName, ToolOutput, ToolSpec, TurnEvent,
};
use serde_json::json;
...
impl SessionStore for MemoryStore {
    ...
    fn save(
        ...
        let revision = SessionRevision(record.revision.0 + 1);
...
fn turn_events_are_ordered_and_release_session_at_terminal_event() {
    ...
    let mut turn = session.prompt("hi").unwrap();
    assert!(session.has_active_turn());
    assert_eq!(
        session.prompt("overlap").unwrap_err(),
    ...
    assert!(!session.has_active_turn());
    assert!(session.prompt("next").is_ok());
}
...
fn dropping_turn_releases_shared_session_lease() {
    ...
    let turn = session.prompt("first").unwrap();
    assert_eq!(clone.prompt("busy").unwrap_err(), EngineError::SessionBusy);
    drop(turn);
    assert!(clone.prompt("released").is_ok());
}
...
fn cancellation_is_idempotent_and_emits_a_terminal_event() {
    ...
    let mut turn = session.prompt("wait").unwrap();
...
fn provider_end_without_stop_is_a_structured_failure_event() {
    ...
    let events = futures_executor::block_on(session.prompt("go").unwrap().collect::<Vec<_>>());
...
fn observer_failure_terminates_and_releases_turn() {
    ...
    let mut turn = session.prompt("go").unwrap();
...
fn assert_turn_surface(engine: &Engine) {
    ...
    let turn = session.prompt("prompt").unwrap();
