...
use machine_god_native::{LIST_FILES_TOOL_NAME, ListFilesTool};
    ...
    fn new() -> Self {
        for _ in 0..1_000 {
            ...
            let path = std::env::temp_dir().join(format!(
                "mg-list-files-engine-{}-{identifier}",
                std::process::id()
            ));
...
fn provider(arguments: Value, name: &str) -> ScriptedModelProvider {
    let call = ToolCall {
        ...
        name: ToolName::new(LIST_FILES_TOOL_NAME).unwrap(),
        arguments,
    };
...
fn collect(engine: &Engine, name: &str) -> (SessionId, Vec<EngineEvent>) {
    ...
    let events = futures_executor::block_on(async {
        session
            .prompt("list the requested directory")
            .await
            .unwrap()
            .collect::<Vec<_>>()
            .await
...
#[test]
fn engine_denial_observes_the_exact_normalized_enumerate_capability_without_leaking_names() {
    let temporary = TemporaryDirectory::new();
    let denied = temporary.path().join("denied");
    fs::create_dir(&denied).unwrap();
    let secret_name = "DENIED_NATIVE_DIRECTORY_SECRET_NAME";
    fs::write(denied.join(secret_name), b"secret contents").unwrap();
    let provider = provider(json!({ "path": "./denied//." }), "native-list-denied");
    ...
    let engine = Engine::builder()
        ...
        .tool(ListFilesTool::open(temporary.path()).unwrap())
        .build()
        .unwrap();

    let (session_id, events) = collect(&engine, "native-list-denied");
    ...
    assert!(!serialized_request(&provider).contains(secret_name));
    assert_eq!(store.record(&session_id).unwrap().messages[2], message);
    assert_eq!(
        fs::read(denied.join(secret_name)).unwrap(),
        b"secret contents"
    );
}

#[test]
fn engine_allow_resolves_policy_before_enumeration_and_durably_returns_exact_output() {
    let temporary = TemporaryDirectory::new();
    let outside = TemporaryDirectory::new();
    let nested = temporary.path().join("nested");
    fs::create_dir(&nested).unwrap();
    fs::create_dir(nested.join("directory")).unwrap();
    fs::write(nested.join("alpha.txt"), b"allowed native contents").unwrap();
    symlink(outside.path(), nested.join("outside-link")).unwrap();
    let provider = provider(json!({ "path": "./nested//." }), "native-list-allowed");
    let store = InMemorySessionStore::new();
    let policy =
    ...
    let (session_id, events) = collect(&engine, "native-list-allowed");

    assert_completed(&events);
    assert_exact_capability(&policy, "nested");
    let resolved = events
        .iter()
        .position(|event| {
            matches!(
    ...
    let expected = ToolOutput::success(json!({
        ...
        "entries": [
            ...
            { "name": "outside-link", "kind": "symlink" }
        ],
        "truncated": false
    }));
    assert!(matches!(
...
fn engine_preflight_error_skips_policy_and_filesystem_execution() {
    ...
    let outside = TemporaryDirectory::new();
    let secret_name = "PREFLIGHT_NATIVE_DIRECTORY_SECRET_NAME";
    fs::write(outside.path().join(secret_name), b"secret contents").unwrap();
    let outside_name = outside.path().file_name().unwrap().to_string_lossy();
    let requested = format!("../{outside_name}");
    let provider = provider(
        json!({ "path": requested }),
        "native-list-preflight-invalid",
    );
    let store = InMemorySessionStore::new();
    ...
    let engine = Engine::builder()
        ...
        .session_store(store.clone())
        .permission_handler(policy.clone())
        .tool(ListFilesTool::open(workspace.path()).unwrap())
        .build()
        .unwrap();

    let (session_id, events) = collect(&engine, "native-list-preflight-invalid");
    ...
    assert_eq!(store.record(&session_id).unwrap().messages[2], message);
    assert_eq!(
        fs::read(outside.path().join(secret_name)).unwrap(),
        b"secret contents"
    );
}
