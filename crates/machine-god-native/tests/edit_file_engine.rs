...
use machine_god_native::{EDIT_FILE_TOOL_NAME, EditFileTool};
...
struct CancelAfterReadyEditTool {
    inner: EditFileTool,
}

impl Tool for CancelAfterReadyEditTool {
    ...
    fn execute(
        ...
        Box::pin(async move {
            ...
            assert!(
                cancellation.cancel(),
                "edit execution was unexpectedly cancelled before becoming ready"
            );
            result
        })
    }
    ...
    fn new() -> Self {
        for _ in 0..1_000 {
            ...
            let path = std::env::temp_dir().join(format!(
                "mg-edit-file-engine-{}-{identifier}",
                std::process::id()
            ));
...
fn provider(path: &str, old_string: &str, new_string: &str, name: &str) -> ScriptedModelProvider {
    let call = ToolCall {
        id: ToolCallId::new("edit-file-call").unwrap(),
        name: ToolName::new(EDIT_FILE_TOOL_NAME).unwrap(),
        arguments: json!({
            "path": path,
...
fn collect(engine: &Engine, name: &str) -> (SessionId, Vec<EngineEvent>) {
    ...
    let events = futures_executor::block_on(async {
        session
            .prompt("edit the requested workspace file")
            .await
            .unwrap()
            .collect::<Vec<_>>()
            .await
...
#[test]
fn engine_denial_uses_normalized_edit_capability_without_reading_or_changing_target() {
    let temporary = TemporaryDirectory::new();
    let target = temporary.path().join("target.txt");
    fs::write(&target, b"private old bytes").unwrap();
    let provider = provider("./target.txt", "old", "new", "native-edit-denied");
    ...
    let engine = Engine::builder()
        ...
        .tool(EditFileTool::open(temporary.path()).unwrap())
        .build()
        .unwrap();

    let (session_id, events) = collect(&engine, "native-edit-denied");
    ...
    assert_eq!(fs::read(&target).unwrap(), b"private old bytes");
}
...
fn engine_allow_resolves_policy_before_events_and_durably_returns_exact_result() {
    ...
    let target = temporary.path().join("nested/file.txt");
    fs::write(&target, "before old after").unwrap();
    let provider = provider(
        "./nested//file.txt",
        "old",
        "allowed λ\0replacement",
        "native-edit-allowed",
    );
    let store = InMemorySessionStore::new();
    let policy =
    ...
    let (session_id, events) = collect(&engine, "native-edit-allowed");
    ...
    let expected_bytes = "before allowed λ\0replacement after";
    let expected = ToolOutput::success(json!({
        "path": "nested/file.txt",
        "bytes_written": expected_bytes.len()
    }));
    assert!(matches!(
    ...
    assert_eq!(store.record(&session_id).unwrap().messages[2], message);
    assert_eq!(fs::read(&target).unwrap(), expected_bytes.as_bytes());
}
...
fn engine_invalid_preflight_skips_policy_filesystem_and_tool_events() {
    ...
    let engine = Engine::builder()
        ...
        .session_store(store.clone())
        .permission_handler(policy.clone())
        .tool(EditFileTool::open(workspace.path()).unwrap())
        .build()
        .unwrap();

    let (session_id, events) = collect(&engine, "native-edit-preflight-invalid");
    ...
    assert_eq!(store.record(&session_id).unwrap().messages[2], message);
    assert_eq!(fs::read(&outside_target).unwrap(), b"outside old");
}
...
fn engine_allowed_execution_error_is_generic_nonretryable_and_durable() {
    ...
    let (session_id, events) = collect(&engine, "native-edit-execution-error");
...
#[test]
fn same_poll_cancellation_after_edit_commit_keeps_durable_unknown_result() {
    let temporary = TemporaryDirectory::new();
    ...
    let provider = provider(
        ...
        "old",
        "committed",
        "native-edit-same-poll-cancellation",
    );
    let store = InMemorySessionStore::new();
    let policy =
        ScriptedPermissionHandler::new([PermissionStep::Decision(PermissionDecision::Allow {
    ...
    let engine = Engine::builder()
        ...
        .session_store(store.clone())
        .permission_handler(policy.clone())
        .tool(CancelAfterReadyEditTool {
            inner: EditFileTool::open(temporary.path()).unwrap(),
        })
        .build()
        .unwrap();

    let (session_id, events) = collect(&engine, "native-edit-same-poll-cancellation");
    ...
    assert_eq!(fs::read(&target).unwrap(), b"committed contents");
    ...
    assert_eq!(call_id.as_str(), "edit-file-call");
    assert_eq!(
        output,
        &ToolOutput {
            content: json!({
