...
#[test]
fn missing_file_uses_schema_v7_built_in_defaults_without_creating_paths() {
    let temporary = TemporaryDirectory::new();
...
#[test]
fn unavailable_home_uses_schema_v7_built_in_defaults() {
...
fn unsupported_schema_version_has_a_distinct_error_kind() {
    let temporary = TemporaryDirectory::new();
    let config_root = temporary.path().join("xdg");
    let contents = r#"{"schema_version":8,"permission_mode":"ask"}"#;
