...
use machine_god_native::{
    ...
    EDIT_FILE_TOOL_NAME, FILE_INFO_TOOL_NAME, GLOB_FILES_TOOL_NAME, GREP_FILES_TOOL_NAME,
    ...
    PermissionPromptError, PermissionPrompter, READ_FILE_TOOL_NAME, WRITE_FILE_TOOL_NAME,
    load_native_config,
};
...
fn assert_exact_native_tool_permissions(prompter: &AllowingPrompter) {
    ...
    assert_eq!(permission_requests.len(), 7);
    assert_eq!(
...
fn assert_persisted_composed_turn(host: &NativeReferenceHost, session_id: SessionId) {
    ...
    assert_eq!(record.messages.len(), 11);
    ...
    assert_eq!(record.messages[10].role, Role::Assistant);
    assert_eq!(
        record.messages[10].content,
        [ContentBlock::Text {
...
fn composition_wires_custom_model_exact_tools_normalized_permissions_and_durable_results() {
    ...
    assert_eq!(third["prompt"].as_array().unwrap().len(), 10);
    ...
    assert_eq!(
        fs::read(nested.join("generated.txt")).unwrap(),
        b"generated edited content"
    );
...
fn replacing_original_workspace_path_cannot_redirect_any_registered_tool() {
    ...
    assert_eq!(
        fs::read(retained.join("nested/generated.txt")).unwrap(),
        b"generated edited content"
    );
    assert!(!workspace.join("nested/generated.txt").exists());
