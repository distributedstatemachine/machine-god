//! Native environment snapshots, configuration preferences, and legacy status.

use crate::{CONFIG_FILE_NAME, STATE_NAMESPACE};
use std::env;
...
impl NativeEnvironment {
    ...
    // Internal consumers share this immutable snapshot without exposing raw
    // environment values through the public API.
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    pub(crate) fn xdg_state_home(&self) -> Option<&OsStr> {
        self.xdg_state_home.as_deref()
    }

    #[cfg(any(target_os = "linux", target_os = "macos"))]
    pub(crate) fn home(&self) -> Option<&OsStr> {
        self.home.as_deref()
    }
...
pub(super) fn resolve_config_file(environment: &NativeEnvironment) -> ResolvedPath {
    resolve_root(
...
pub(super) fn resolve_state_directory(environment: &NativeEnvironment) -> ResolvedPath {
    resolve_root(
...
pub(super) fn inspect_state_directory(path: &Path) -> StateDirectoryState {
    match path.symlink_metadata() {
...
mod tests {
    use super::{
        ...
        StateDirectoryState, inspect_native_status,
    };
    use crate::SUPPORTED_CORE_API_VERSION;
