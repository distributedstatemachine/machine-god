...
use super::{ProfileFileError as Error, READ, validate_link, validate_private};
use rustix::fd::OwnedFd;
use rustix::fs::{AtFlags, Mode, OFlags};
use std::ffi::OsString;
use std::path::{Path, PathBuf};

pub(crate) struct ParentObservation {
    ancestor: OwnedFd,
    identity: PathBuf,
...
impl ParentObservation {
    pub(crate) fn observe(path: &Path) -> Result<Self, Error> {
    ...
    pub(crate) fn resolve(&self, create: bool) -> Result<Option<ResolvedParent<'_>>, Error> {
        self.resolve_with_sync(create, |fd| rustix::fs::fsync(fd))
    }

    pub(crate) fn resolve_with_sync(
        &self,
        create: bool,
