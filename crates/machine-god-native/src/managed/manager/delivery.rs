    ...
    pub(super) fn poll_delivery_clears(&mut self, cx: &mut Context<'_>) -> bool {
        ...
        for parent in &mut self.parents {
            ...
            if parent.clearing.is_none()
                ...
                && let Some(child) = self.children.iter().find(|child| {
                    !child.busy()
                        && !child.closing
                        && child
                            .prepared
                            .notice_context
                            .as_ref()
                            .is_some_and(|context| parent.context.ptr_eq(&Arc::downgrade(context)))
                })
            {
                let runtime = child.prepared.runtime.clone();
            ...
            for record in page.records {
                match record {
                            ...
                            if control.revision == original.target.relationship_generation.get() {
                                ...
                                lineage[index] =
                                    control.parent_owner.as_ref().is_some_and(|owner| {
                                        owner.session_id == checkpoint.session_id
                                            && owner.incarnation == checkpoint.incarnation_id
                                    });
                            }
                        }
                    }
                    JournalRecord::Notice(notice) => {
                        if let Some(index) =
                            selected.iter().position(|original| **original == notice)
                        {
                            found[index] = true;
                        }
                    }
                    JournalRecord::NoticeAcknowledged {
                        ...
                        target,
                        checkpoint,
                    } if &checkpoint == delivery.checkpoint() => {
