// Included in terminal_host::tests to exercise the actual complete host.
use super::*;
...
use crate::{NativeSandboxMode, NativeWorkspaceAuthority, NativeWorkspaceContexts,
    NativeWorkspaceEntrySpec, NativeWorkspaceSource, PermissionMode};
...
impl Scoped {
    fn new(mode: Option<(PermissionMode, NativeSandboxMode)>, tmux: Option<PathBuf>) -> Self {
        let (permission_mode, sandbox_mode) = mode.unwrap_or((PermissionMode::Ask, NativeSandboxMode::None));
        ...
        let mut host = Fixture::with_workspace(Some(contexts.clone()), tmux, |workspace| {
            ...
            let scope = NativeWorkspaceAuthority::open_blocking(
                open(workspace), workspace.to_owned(), Some(open(&base.join("state"))), base.join("state"),
            ...
            mode.map(|_| {
                let executable = if cfg!(target_os="macos") && sandbox_mode == NativeSandboxMode::Os {
                    Some(std::fs::File::open(crate::NATIVE_SANDBOX_EXECUTABLE).unwrap())
                } else { None };
                let selected = Arc::new(NativeTerminalPermissionPolicy::new(vec![], executable).unwrap().with_workspace_contexts(contexts.clone()));
                selected.bind_controller(&policy.controller).unwrap();
                selected
            })
        });
        host.context = context;
        Self { host, _policy:policy, turn, _permission:permission, owner, registration,
            authority:authority.unwrap(), contexts }
    }

    fn extra(&self) -> PathBuf { self.host.root.join("extra") }

    fn reregister(&mut self) {
        self.registration.take();
        self.registration = Some(self.owner.begin(&self.turn, self.authority.snapshot().unwrap()).unwrap());
    }

    fn prepared(&self, arguments: Value) -> machine_god_core::PreparedToolCall {
        self.host.tool.prepare(ToolCall { id:self.host.context.call_id.clone(), name:ToolName::new("terminal").unwrap(), arguments }).unwrap()
    }
}
...
fn workspace_cwd_exec_preserves_native_symlink_parent_and_old_snapshot() {
    ...
    std::os::unix::fs::symlink(fixture.extra().join("child"),fixture.host.root.join("workspace/link")).unwrap();
    for cwd in [fixture.extra().to_str().unwrap(), "link/.."] {
        ...
        assert_eq!(std::fs::read(fixture.extra().join("native-parent")).unwrap(),b"retained");
    }
    fixture.authority.install(fixture.authority.prepare_blocking(vec![],false).unwrap()).unwrap();
    fixture.host.action(json!({"action":"exec","profile":"clean","cwd":fixture.extra(),"command":"printf old > old-snapshot"}));
    assert_eq!(std::fs::read(fixture.extra().join("old-snapshot")).unwrap(),b"old");
...
fn workspace_cwd_unpolled_entrypoints_cannot_rebind_reused_registration() {
    for for_turn in [false,true] {
        ...
        if for_turn {
            let future = fixture.host.tool.execute_for_turn(fixture.host.context.clone(),prepared.arguments().clone(),CancellationToken::new());
            fixture.registration.take();
            fixture.registration = Some(fixture.owner.begin(&fixture.turn,fixture.authority.snapshot().unwrap()).unwrap());
            assert!(futures_executor::block_on(future).is_err());
        } else {
            let future = fixture.host.tool.execute(fixture.host.context.clone(),prepared.arguments().clone(),CancellationToken::new());
            fixture.registration.take();
            fixture.registration = Some(fixture.owner.begin(&fixture.turn,fixture.authority.snapshot().unwrap()).unwrap());
            assert!(futures_executor::block_on(future).is_err());
        }
...
fn workspace_cwd_governed_wrapper_keeps_both_acceptance_stamps() {
    for for_turn in [false,true] {
        let mut fixture = Scoped::new(None,None);
        let tool = crate::NativePermissionGovernedTool::new(fixture.host.tool.clone(), machine_god_core::EngineLimits::default());
        ...
        if for_turn {
            let future = tool.execute_for_turn(fixture.host.context.clone(),prepared.arguments().clone(),CancellationToken::new());
            fixture.reregister();
            assert!(futures_executor::block_on(future).is_err());
        } else {
            let future = tool.execute(fixture.host.context.clone(),prepared.arguments().clone(),CancellationToken::new());
            fixture.reregister();
            assert!(futures_executor::block_on(future).is_err());
        }
...
fn workspace_cwd_permission_evidence_uses_same_scope_and_rejects_late_registration() {
    use machine_god_core::{PermissionInvocation,PermissionRequest,PermissionRequestId,PermissionRisk};
    let mut fixture = Scoped::new(None,None);
    let primary = fixture.host.root.join("workspace");
    let authority = crate::NativePermissionTargetAuthority::new(std::fs::File::open(&primary).unwrap(),primary.to_str().unwrap().into(),vec![crate::NativePermissionTargetTool::Terminal(fixture.host.tool.clone())]).unwrap().with_workspace_contexts(fixture.contexts.clone());
    let prepared = fixture.prepared(json!({"action":"exec","profile":"clean","cwd":fixture.extra(),"command":"printf no > permission-must-not-run"}));
    let request = PermissionRequest { id:PermissionRequestId::new("cwd").unwrap(),session_id:fixture.host.context.session_id.clone(),session_incarnation_id:fixture.host.context.session_incarnation_id.clone(),turn_id:fixture.host.context.turn_id.clone(),capability:prepared.capability().unwrap().clone(),risk:PermissionRisk::Critical,reason:"test".into() };
    let name = ToolName::new("terminal").unwrap();
    let call_id = fixture.host.context.call_id.clone();
    let invocation = || PermissionInvocation { tool_name:&name,call_id:&call_id,arguments:prepared.arguments() };
    let old = authority.prepare(&request,invocation(),CancellationToken::new());
    fixture.reregister();
    assert!(futures_executor::block_on(old).is_err());
    let evidence = futures_executor::block_on(authority.prepare(&request,invocation(),CancellationToken::new())).unwrap();
    let Some(TerminalActionRequest::Exec {request}) = evidence.terminal_action() else { panic!("exec evidence"); };
    ...
    let scope = Arc::new(foreign.contexts.snapshot_for_tool(&foreign.host.context).unwrap());
    ...
    assert!(futures_executor::block_on(resolver.resolve_with_workspace_scope(invocation,Some(scope),CancellationToken::new())).is_err());
    fixture.reregister();
...
fn workspace_cwd_missing_foreign_removed_replaced_and_cancelled_scopes_fail_closed() {
    ...
    assert!(futures_executor::block_on(fixture.host.tool.execute(wrong,prepared.arguments().clone(),CancellationToken::new())).is_err());
    fixture.registration.take();
    let absent = fixture.host.tool.execute(fixture.host.context.clone(),prepared.arguments().clone(),CancellationToken::new());
    fixture.registration = Some(fixture.owner.begin(&fixture.turn,fixture.authority.snapshot().unwrap()).unwrap());
    ...
    assert!(futures_executor::block_on(fixture.host.tool.execute(fixture.host.context.clone(),prepared.arguments().clone(),CancellationToken::new())).is_err());
    fixture.authority.install(fixture.authority.prepare_blocking(vec![],false).unwrap()).unwrap();
    fixture.reregister();
    assert!(futures_executor::block_on(fixture.host.tool.execute(fixture.host.context.clone(),prepared.arguments().clone(),CancellationToken::new())).is_err());
    fixture.turn.handle().cancel();
    assert!(fixture.host.try_action(json!({"action":"exec","profile":"clean","command":"printf no > forbidden"})).is_err());
...
fn workspace_cwd_none_yolo_and_os_keep_distinct_launch_policy() {
    let mut modes = vec![(PermissionMode::Ask,NativeSandboxMode::None),(PermissionMode::Yolo,NativeSandboxMode::Os)];
    if cfg!(target_os="macos") { modes.push((PermissionMode::Auto,NativeSandboxMode::Os)); }
    for mode in modes {
        ...
        assert_eq!(std::fs::read(fixture.extra().join("mode")).unwrap(),b"allowed");
    }
}
...
fn workspace_cwd_tmux_launch_uses_additional_directory() {
    ...
    let executable = explicit.clone().map(PathBuf::from).or_else(|| ["/opt/homebrew/bin/tmux","/usr/bin/tmux","/usr/local/bin/tmux"].into_iter().map(PathBuf::from).find(|path|path.is_file()));
    let Some(executable) = executable else { assert!(explicit.is_none()); return; };
